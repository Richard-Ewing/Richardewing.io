...
export default function VoiceDock() {
  ...
  // Initialize Speech Synthesis and check support on client
  useEffect(() => {
  ...
  // Session timer: 90-second cap
  useEffect(() => {
    if (isOpen && status !== 'limit_reached') {
    ...
    try {
      ...
      const naturalVoice = voices.find(
        (v) =>
          ...
          (v.name.includes('Natural') || v.name.includes('Google') || v.name.includes('Alex'))
      );
      if (naturalVoice) {
  ...
  const handleSendMessage = async (text: string) => {
    if (!text.trim() || status === 'limit_reached') return;

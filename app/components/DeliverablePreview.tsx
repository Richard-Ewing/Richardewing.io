                ...
                <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 p-1 rounded-xl">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-colors ${
                            activeTab === 'brief' ? 'bg-violet-600 text-white' : 'text-zinc-400 hover:text-zinc-200'
                        }`}
                    >
                        Risk Report PDF
                    </button>
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-colors ${
                            activeTab === 'pdi' ? 'bg-violet-600 text-white' : 'text-zinc-400 hover:text-zinc-200'
                        }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition-colors ${
                            activeTab === 'board' ? 'bg-violet-600 text-white' : 'text-zinc-400 hover:text-zinc-200'
                        }`}
                    >
            ...
            <div className="bg-zinc-900/90 border border-zinc-800 rounded-2xl p-6 relative min-h-[280px]">
                {activeTab === 'brief' && (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        <div className="flex justify-between items-start border-b border-zinc-800 pb-4">
                            <div>
                                <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">SAMPLE DOCUMENT OUTPUT</span>
                                <h4 className="text-lg font-bold text-white">R&D Capital & AI Unit Economics Audit</h4>
                                <p className="text-xs text-zinc-400">Prepared for: Board of Directors & C-Suite</p>
                            </div>
                            <span className="px-2 py-1 rounded text-[10px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                                CONFIDENTIAL
                            </span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                            <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                                <span className="text-zinc-500 block mb-1">Identified Capital Leakage</span>
                                <span className="text-xl font-bold font-mono text-rose-400">$840,000 / yr</span>
                                <p className="text-[10px] text-zinc-500 mt-1">Ungoverned retries & zombie features</p>
                            </div>
                            <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                                <span className="text-zinc-500 block mb-1">Technical Insolvency Date</span>
                                <span className="text-xl font-bold font-mono text-amber-400">Q3 2027</span>
                                <p className="text-[10px] text-zinc-500 mt-1">Point where maintenance outpaces innovation</p>
                            </div>
                            <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                                <span className="text-zinc-500 block mb-1">Remediation Margin Upside</span>
                                <span className="text-xl font-bold font-mono text-emerald-400">+34% Net ROI</span>
                                <p className="text-[10px] text-zinc-500 mt-1">Via VPC model repatriation & cost-caps</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'pdi' && (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        <div className="flex justify-between items-start border-b border-zinc-800 pb-4">
                            <div>
                                <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">PRODUCT DEBT INDEX (PDI) MATRIX</span>
                        ...
                        <div className="space-y-2 text-xs font-mono">
                            {[
                                ...
                                <div key={idx} className="flex justify-between items-center bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                                    <span className="text-zinc-300 font-sans font-medium">{row.dim}</span>
                                    <div className="flex items-center gap-4">
                                        <span className="font-bold text-white">{row.score}</span>
                                        <span className={`text-[10px] uppercase ${row.color}`}>{row.status}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeTab === 'board' && (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        <div className="border-b border-zinc-800 pb-4">
                            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">EXECUTIVE BRIEF</span>
                            <h4 className="text-lg font-bold text-white">1-Page Board Executive Briefing</h4>
                        </div>
                        <blockquote className="border-l-2 border-violet-500 pl-4 py-1 text-sm text-zinc-300 italic">

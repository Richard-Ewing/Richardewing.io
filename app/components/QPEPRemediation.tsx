...
export function QPEPRemediation({ toolId, metrics }: QPEPRemediationProps) {
    ...
    return (
        <section className="mt-8 bg-gradient-to-br from-zinc-900/80 via-zinc-900/40 to-transparent border border-white/10 rounded-3xl p-6 sm:p-8">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-1 tracking-tight">Prescriptive Action Plan</h3>
            <p className="text-xs text-zinc-500 mb-6">Qualitative-Profitability Efficiency Protocol — Generated {new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                        ...
                        <div key={i} className={`${colors.bg} border ${colors.border} rounded-2xl p-5`}>
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-2">
                                    <span className="font-mono text-[10px] text-zinc-500 tracking-widest">{item.phase}</span>
                                    <span className="text-zinc-700">|</span>
                                    <span className={`font-mono text-[10px] ${colors.text} tracking-widest font-bold`}>{item.urgency}</span>
                                </div>
                                {item.asc_reference && (
                                    <span className="font-mono text-[9px] text-zinc-600 bg-zinc-800/60 px-2 py-1 rounded-md">{item.asc_reference}</span>
                                )}
                            </div>
                            <h4 className="text-sm font-bold text-white mb-2">{item.title}</h4>
                            <p className="text-xs text-zinc-400 leading-relaxed">{item.prescription}</p>
                        </div>
                    );
                })}
            </div>

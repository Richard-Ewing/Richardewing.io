...
export function GatewayCard({ title, href, color = 'cyan', description = "Launch Tool" }: { title: string, href: string, color?: 'cyan' | 'red' | 'violet', description?: string }) {
    const colors = {
        cyan: 'border-cyan-500/30 bg-cyan-50/20 text-cyan-900 font-extrabold font-semibold hover:border-cyan-400',
        red: 'border-red-500/30 bg-red-50/20 text-red-900 font-extrabold font-semibold hover:border-red-400',
    ...
    return (
        <Link href={href} className={`block w-full p-8 rounded-2xl border transition-all duration-300 group ${activeColor}`}>
            <div className="flex items-center justify-between">
                <div>
                    <div className="font-mono text-xs font-bold uppercase tracking-widest opacity-70 mb-2">Interactive Engine</div>
            ...
            <div className="mt-4 text-sm font-semibold font-mono opacity-80 flex items-center gap-2">

    ...
    return (
        <div className="max-w-6xl w-full relative z-10 mx-auto">
            ...
            <div className="mb-6 flex items-center gap-2 text-xs font-bold font-medium font-mono text-zinc-950 font-bold uppercase tracking-widest">
                <span>Knowledge</span><span>/</span><span className="text-cyan-900 font-extrabold font-semibold font-bold">Canonical Hub</span>
            </div>
                ...
                <div className="mb-12 border-b border-zinc-400 pb-12">
                    ...
                    <div className="flex flex-wrap gap-4 mt-8">
                        {['Frameworks', 'Publications'].map(link => (
                            <a href={`#${link.toLowerCase()}`} key={link} className="text-xs font-bold uppercase tracking-widest text-zinc-950 font-bold hover:text-cyan-900 font-extrabold font-semibold transition-colors">
                                [{link}]
                            </a>
                        ))}
                    </div>
            ...
            <ScrollReveal delay={100}>
                <div className="mb-20">
                    <h3 className="font-mono text-xs font-bold text-zinc-900 font-bold uppercase tracking-widest mb-4">Featured</h3>
                    <div className="card-featured p-8 md:p-12 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 bg-purple-500/20 text-purple-900 font-extrabold font-semibold px-4 py-1 text-xs font-bold uppercase tracking-widest rounded-bl-xl border-l border-b border-purple-500/30">
                            Editor's Pick
                        </div>

                        <div className="relative z-10">
                            <div className="flex items-center gap-3 mb-4 text-xs font-bold font-mono text-zinc-900 uppercase tracking-widest">
                                <span className="text-zinc-900">{featuredArticle.publication}</span>
                                <span>•</span>
                                <span>{featuredArticle.date}</span>
                            </div>

                            <h2 className="text-3xl md:text-4xl font-bold text-zinc-950 mb-4 font-grotesk group-hover:text-purple-900 font-extrabold font-semibold transition-colors">
                            ...
                            <a href={featuredArticle.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-purple-900 font-extrabold font-semibold font-bold uppercase tracking-widest text-xs font-bold hover:text-zinc-900 transition-colors">
                        ...
                        {frameworks.map((fw, i) => (
                            ...
                            >
                                <div className="card h-full p-6 hover:border-cyan-500/50 transition-all flex flex-col">
                                    <h3 className="text-xl font-bold text-zinc-950 mb-3 group-hover:text-cyan-900 font-extrabold font-semibold transition-colors font-grotesk">
                                        {fw.name}
                                    </h3>
                                    <p className="text-sm font-semibold text-zinc-900 font-medium mb-6 flex-grow line-clamp-3">
                                        {fw.definition}
                                    </p>
                                    <div className="text-xs font-bold uppercase tracking-widest text-cyan-500 group-hover:text-cyan-900 font-extrabold font-semibold flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            ...
                            >
                                ...
                                <div className={`text-xs font-bold font-mono uppercase tracking-widest transition-colors w-full text-center ${selectedPublication === pub.name ? 'text-cyan-900 font-extrabold font-semibold' : 'text-zinc-950 group-hover:text-zinc-950 font-bold'}`}>
            ...
            <ScrollReveal delay={400}>
                <div id="article-list" className="mb-20">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {selectedPublication && (
                            <button onClick={() => setSelectedPublication(null)} className="text-xs font-bold text-zinc-900 font-bold hover:text-red-900 font-extrabold font-semibold uppercase tracking-widest font-bold">
                                Clear Filter ✕
                            </button>
                        )}
                    </div>
                    ...
                    {displayedArticles.length > 0 ? (
                                        ...
                                        <div>
                                            <div className="flex justify-between items-center mb-4">
                                                <div className="flex items-center gap-2">
                                                    <span className="font-mono text-xs font-bold font-medium text-zinc-900 uppercase tracking-widest border border-zinc-400 px-2 py-0.5 rounded-full">{article.source}</span>
                                                </div>
                                                <span className="font-mono text-xs font-bold text-zinc-900 font-bold uppercase tracking-widest">{article.date}</span>
                                            </div>
                                            <h3 className="text-xl font-bold text-zinc-950 mb-2 group-hover:text-cyan-900 font-extrabold font-semibold transition-colors font-grotesk">{article.title}</h3>
                        ...
                        <div className="text-center py-20 border border-dashed border-zinc-400 rounded-2xl">
                            ...
                            <button onClick={() => setSelectedPublication(null)} className="mt-4 text-cyan-900 font-extrabold font-semibold hover:text-cyan-900 font-extrabold font-semibold text-sm">Clear Filters</button>
                        </div>
                    )}

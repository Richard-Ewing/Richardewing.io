...
export default function NegativeCarryCodeAuditorTool() {
    ...
    const negativeCarryRiskScore = Math.min(100, Math.round(testPenalty + aiRiskFactor + regressionPenalty));
    const maintenanceCapacityPercent = Math.min(95, Math.round(35 + (negativeCarryRiskScore * 0.55)));
    ...
    if (negativeCarryRiskScore < 35) {
        riskTier = 'Low Negative-Carry Risk (Well-Governed)';
        riskBadgeColor = 'bg-emerald-50 text-emerald-800 border-emerald-200';
    } else if (negativeCarryRiskScore < 60) {
        riskTier = 'Moderate Negative-Carry Accumulation';
        riskBadgeColor = 'bg-amber-50 text-amber-800 border-amber-200';
    }

    return (
        <ToolGate toolName="Negative-Carry Code Auditor">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8" id="negative-carry-code-calc">
                {/* Header */}
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl sm:text-5xl font-extrabold font-grotesk text-zinc-950 tracking-tight">
                        Negative-Carry Code <span className="text-rose-600">Auditor (NCCA)</span>
                    </h1>
                    <p className="mt-3 text-base text-zinc-700 max-w-3xl mx-auto font-medium leading-relaxed">
                        When teams generate code 4x faster with unconstrained AI without writing unit tests, they create &ldquo;negative-carry code&rdquo; - fast to scaffold, but carrying an ongoing maintenance tax that exceeds its economic value. Calculate the balance-sheet refactor discount.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
                        <div className="bg-white border-2 border-zinc-900 rounded-3xl p-6 sm:p-8 shadow-md space-y-6">
                            <div>
                                <div className="text-xs font-mono font-bold text-zinc-500 uppercase tracking-widest mb-1">Negative-Carry Risk Score</div>
                                <div className="flex items-baseline gap-3">
                                    <div className="text-5xl font-extrabold font-grotesk text-rose-600">{negativeCarryRiskScore}</div>
                            ...
                            <div className="pt-2 border-t border-zinc-200 flex items-center justify-between">
                                ...
                                <ExportToPDFButton targetId="negative-carry-code-calc" fileName="negative-carry-code-risk-audit.pdf" />
                            </div>
                        </div>
                        ...
                        <div className="p-5 rounded-2xl bg-rose-50/70 border border-rose-200 text-xs space-y-2">
                            ...
                            <p className="text-rose-900 leading-relaxed">
                                Directly maps to <Link href="/concepts/negative-carry-code-crisis" className="underline font-bold">Negative-Carry Code Crisis</Link> and <Link href="/concepts/vibe-coding" className="underline font-bold">Vibe Coding Debt</Link>.
                            </p>
                        </div>
                    </div>
                </div>

    ...
    return (
        <div className="max-w-5xl w-full relative z-10 mx-auto px-4">
            ...
            {!results ? (
                <ScrollReveal>
                    <div className="capsule-container rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 mb-8 border border-zinc-400">
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <span className="font-mono text-xs font-bold text-zinc-900 font-bold uppercase tracking-widest">AI Unit Economics | Build vs Buy</span>
                        </div>
                        ...
                        <div className="mb-8">
                            <div className="text-xs font-bold font-mono text-zinc-950 font-bold uppercase tracking-widest mb-3">I am calculating for the...</div>
                            <div className="flex flex-wrap gap-2">
                                {PERSONAS.map(p => (
                                    <button key={p.id} onClick={() => setPersona(p.id)}
                                        className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-all ${persona === p.id ? 'bg-emerald-500/10 border-emerald-500 text-emerald-900 font-extrabold font-semibold' : 'bg-white/80 border-zinc-400 text-zinc-900 hover:border-white/30'}`}
                                    >
                        ...
                        <div className="space-y-6">
                            {step === 1 && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                                    <div className="flex items-center gap-3 border-b border-zinc-400 pb-4">
                                        <div className="w-8 h-8 rounded-full bg-blue-500/20 text-blue-900 font-extrabold font-semibold flex items-center justify-center font-bold font-mono text-sm font-semibold border border-blue-500/30">1</div>
                                        <div>
                                            <h3 className="text-xl font-bold text-zinc-900">Inference Volume</h3>
                                            <p className="text-sm font-semibold text-zinc-900 font-medium">How heavy is your generative payload?</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-8">
                                        <div className="p-6 bg-zinc-50 rounded-xl border border-zinc-400 relative group">
                                            <div className="flex justify-between items-end mb-4">
                                                <label className="text-xs font-bold font-mono text-blue-900 font-extrabold font-semibold uppercase tracking-widest flex items-center gap-2">
                                                    Generative Workflows / Day
                                                    <span className="w-4 h-4 rounded-full bg-zinc-200 text-zinc-900 flex items-center justify-center text-xs font-bold font-medium cursor-help" title="How many times users trigger an LLM (chat, summarization, workflow automation)">?</span>
                                                </label>
                                        ...
                                        <div className="p-6 bg-zinc-50 rounded-xl border border-zinc-400 relative group">
                                            <div className="flex justify-between items-end mb-4">
                                                <label className="text-xs font-bold font-mono text-indigo-900 font-extrabold font-semibold uppercase tracking-widest flex items-center gap-2">
                                                    ...
                                                    <span className="w-4 h-4 rounded-full bg-zinc-200 text-zinc-900 flex items-center justify-center text-xs font-bold font-medium cursor-help" title="RAG payload: User prompt + DB Context + LLM Output. 1000 tokens ≈ 750 words.">?</span>
                                                </label>
                                                <div className="text-2xl font-bold text-zinc-950 font-mono">{tokensPerQuery.toLocaleString()} t/req</div>
                                            </div>
                                            <input type="range" title="Tokens Per Workflow" aria-label="Tokens Per Workflow" min="500" max="32000" step="500" value={tokensPerQuery} onChange={e => setTokensPerQuery(parseInt(e.target.value))} className="w-full h-2 bg-zinc-200 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                                        </div>
                                    </div>
                            ...
                            {step === 2 && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                                    <div className="flex items-center gap-3 border-b border-zinc-400 pb-4">
                                        <div className="w-8 h-8 rounded-full bg-emerald-500/20 text-emerald-900 font-extrabold font-semibold flex items-center justify-center font-bold font-mono text-sm font-semibold border border-emerald-500/30">2</div>
                                        <div>
                                            <h3 className="text-xl font-bold text-zinc-900">Vendor & Target Hardware</h3>
                                            <p className="text-sm font-semibold text-zinc-900 font-medium">Choose your API dependency and local hardware alternative.</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            <label className="text-xs font-bold font-mono text-zinc-950 font-bold uppercase tracking-widest mb-3 block"><BrainCircuit size={12} className="inline mr-1"/> Current API Tax Collector</label>
                                        ...
                                        <div>
                                            <label className="text-xs font-bold font-mono text-zinc-950 font-bold uppercase tracking-widest mb-3 block"><HardDrive size={12} className="inline mr-1"/> Local SLM Strategy (vLLM/Ollama)</label>
                                            ...
                                            <p className="text-xs font-bold font-medium text-zinc-950 font-bold font-mono mt-2">Includes amortized $150/hr DevOps maintenance tax.</p>
                                        </div>
                                    </div>
                ...
                <div id="slm-results-artifact" className="bg-white p-2 sm:p-6 rounded-3xl">
                    <div className="flex flex-col sm:flex-row items-center justify-between bg-white/60 border border-emerald-500/20 rounded-2xl p-6 mb-8 backdrop-blur-md">
                        <div>
                            ...
                            <p className="text-sm font-semibold text-zinc-900 font-medium">Your AI unit economics have been quantified.</p>
                        </div>
                    ...
                    <div id="slm-pdf-export-zone" className="space-y-6">
                        <ScrollReveal>
                            <div className="capsule-container rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 mb-6 relative overflow-hidden border border-zinc-400">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center relative z-10">
                                    <div>
                                        <div className="text-xs font-bold font-mono text-zinc-950 font-bold uppercase tracking-widest mb-2">36-Month Strategic Verdict</div>
                                        ...
                                        <div className="mt-6 flex flex-col gap-2">
                                            <span className="text-zinc-900 font-mono text-sm font-semibold leading-relaxed">
                                    ...
                                    <div>
                                        <div className="bg-white/50 p-6 rounded-2xl border border-zinc-400">
                                            {results.isSelfHostCheaper ? (
                                                <>
                                                    <div className="text-xs font-bold font-mono text-emerald-500 uppercase tracking-widest mb-1">Projected Annual Savings</div>
                                                    <div className="text-5xl font-bold text-emerald-900 font-extrabold font-semibold font-mono mb-2">+{formatMoney(results.annualSavings)}</div>
                                                    <p className="text-xs font-bold text-zinc-900 font-bold">By migrating to {results.hardwareName}, you break even on hardware and maintenance in <strong className="text-zinc-900">{results.breakevenDays} days</strong> of operation.</p>
                                                </>
                                            ) : (
                                                <>
                                                    <div className="text-xs font-bold font-mono text-red-500 uppercase tracking-widest mb-1">Local Hosting Premium</div>
                                                    <div className="text-5xl font-bold text-red-900 font-extrabold font-semibold font-mono mb-2">-{formatMoney(Math.abs(results.annualSavings))}</div>
                                                    <p className="text-xs font-bold text-zinc-900 font-bold">You do not have the token volume to justify {results.hardwareName} and a local MLOps team overhead.</p>
                                                </>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <ScrollReveal delay={100}>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                                <GlowCard className="p-6" glowColor="blue">
                                    <div className="text-xs font-bold font-medium font-mono text-zinc-950 font-bold uppercase tracking-widest mb-2">API Run Rate</div>
                                    <div className="text-3xl font-bold text-blue-900 font-extrabold font-semibold">{formatMoney(results.apiCost)}<span className="text-sm font-semibold font-normal text-zinc-900">/mo</span></div>
                                    <p className="text-xs font-bold font-medium text-zinc-950 mt-2 line-clamp-2">Paid to LLM vendor directly via metered billing.</p>
                                </GlowCard>

                                <GlowCard className="p-6" glowColor="emerald">
                                    <div className="text-xs font-bold font-medium font-mono text-zinc-950 font-bold uppercase tracking-widest mb-2">Total Local Cost</div>
                                    <div className="text-3xl font-bold text-emerald-900 font-extrabold font-semibold">{formatMoney(results.localCost)}<span className="text-sm font-semibold font-normal text-zinc-900">/mo</span></div>
                                    <p className="text-xs font-bold font-medium text-zinc-950 mt-2 line-clamp-2">{results.hardwareName} + Devops baseline overhead.</p>
                                </GlowCard>

                                <GlowCard className="p-6" glowColor={results.isSelfHostCheaper ? "emerald" : "red"}>
                                    <div className="text-xs font-bold font-medium font-mono text-zinc-950 font-bold uppercase tracking-widest mb-2">Arbitrage Gap</div>
                                    <div className="text-3xl font-bold text-zinc-900">{formatMoney(Math.abs(results.monthlySavings))}<span className="text-sm font-semibold font-normal text-zinc-900">/mo</span></div>
                                    <p className="text-xs font-bold font-medium text-zinc-950 mt-2 line-clamp-2">The exact financial gap between architectures.</p>
                                </GlowCard>
                            </div>
                        </ScrollReveal>
                        ...
                        <ScrollReveal delay={150}>
                             <div className="bg-gradient-to-br from-zinc-50 via-zinc-100 to-zinc-50/60 rounded-2xl p-6 border border-zinc-400 mb-8">
                                <div className="flex items-center gap-2 mb-4">
                                    <Zap size={16} className="text-yellow-900 font-extrabold font-semibold" />
                                    <span className="text-xs font-bold font-mono uppercase tracking-widest text-zinc-900">The "Why" For Private AI</span>
                                </div>
                        ...
                        <ScrollReveal delay={180}>
                            <div className="capsule-container rounded-2xl p-6 sm:p-8 mb-8 border border-zinc-400 bg-zinc-50 text-left">
                                ...
                                <p className="text-zinc-900 text-sm font-semibold mb-8">Execute this operational sequence immediately to arrest runaway LLM infrastructure costs and restore your engineering unit economics.</p>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-white/80 border border-zinc-400 rounded-xl p-5 flex flex-col sm:flex-row gap-5 items-start border-l-2 border-l-amber-500 relative overflow-hidden group hover:bg-zinc-100 transition-colors shrink-0">
                                        ...
                                        <div className="relative z-10 w-full">
                                            ...
                                            <p className="text-zinc-900 text-sm font-semibold leading-relaxed mb-4">You are likely paying frontier-model APIs repeatedly to generate responses to identical user queries. Token taxation on cached queries is a massive EBITDA leak.</p>
                                            <div className="bg-white/60 p-3 rounded border border-zinc-400 flex flex-col gap-2">
                                                ...
                                                <p className="text-sm font-semibold font-medium text-zinc-950">Deploy a vector similarity cache (Redis / Pinecone) in front of all generative endpoints. Target a 30% hit rate reduction on outbound network requests by end of quarter.</p>
                                            </div>
                                        </div>
                                    </div>

                                    {/* Step 3 */}
                                    <div className="bg-white/80 border border-zinc-400 rounded-xl p-5 flex flex-col sm:flex-row gap-5 items-start border-l-2 border-l-emerald-500 relative overflow-hidden group hover:bg-zinc-100 transition-colors shrink-0">
                                        ...
                                        <div className="bg-emerald-500/10 w-12 h-12 rounded-lg flex items-center justify-center shrink-0 border border-emerald-500/20">
                                            <span className="text-emerald-900 font-extrabold font-semibold font-bold font-mono">03</span>
                                        </div>
                                        <div className="relative z-10 w-full">
                                            ...
                                            <div className="bg-white/60 p-3 rounded border border-zinc-400 flex flex-col gap-2">
                                                ...
                                                <p className="text-sm font-semibold font-medium text-zinc-950">Stand up a local bare-metal node running vLLM. Shadow-route 10% of API traffic to the SLM to prove deterministic latency and output parity before the board review.</p>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <ScrollReveal delay={250}>
                            <div className="flex justify-center gap-6 mt-6" data-html2canvas-ignore>
                                <button onClick={() => setResults(null)} className="text-zinc-950 text-sm font-semibold hover:text-zinc-900 underline underline-offset-4">← Re-Calculate Variables</button>
                            </div>
                        </ScrollReveal>
                    </div>
                </div>

            ...
            <div className="page-container">
                <div className="max-w-4xl mx-auto">
                    ...
                    <h1 className="text-4xl sm:text-5xl font-grotesk font-bold text-white mb-6">
                        EV-SE <span className="text-zinc-500">vs</span>{' '}
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-cyan-400">Jellyfish</span>
                    </h1>
                    <p className="text-lg text-zinc-400 mb-12 max-w-2xl">
                        Jellyfish tracks what engineers are working on. EV-SE tells you <span className="text-white font-bold">whether that work is economically sustainable</span>. Activity metrics ≠ economic sustainability.
                    </p>
                    <div className="overflow-x-auto mb-16">
                        <table className="w-full border-collapse">
                            <thead><tr className="border-b border-zinc-200"><th className="text-left py-4 px-4 text-xs font-mono text-zinc-500 uppercase tracking-widest">Dimension</th><th className="text-left py-4 px-4 text-xs font-mono text-teal-400 uppercase tracking-widest">EV-SE</th><th className="text-left py-4 px-4 text-xs font-mono text-zinc-400 uppercase tracking-widest">Jellyfish</th></tr></thead>
                            <tbody>{rows.map((row, i) => (<tr key={i} className="border-b border-zinc-200 hover:bg-white/[0.02]"><td className="py-4 px-4 text-sm text-zinc-400 font-bold">{row.dimension}</td><td className="py-4 px-4 text-sm text-white">{row.evse}</td><td className="py-4 px-4 text-sm text-zinc-400">{row.jelly}</td></tr>))}</tbody>
                        </table>
                    </div>
                    <div className="rounded-2xl border border-teal-500/30 bg-teal-500/5 p-8 mb-16">
                        ...
                        <p className="text-zinc-300 mb-6"><strong>Use both — for different purposes.</strong> Jellyfish shows engineering activity. EV-SE evaluates whether that activity creates lasting value. Tracking without economic analysis is measurement without meaning.</p>

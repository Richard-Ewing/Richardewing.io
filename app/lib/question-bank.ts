...
def enqueue(task):
    ...
    return {"status": "queued"}`,
                ...
                },
                {
                    id: "eng_p2_v5",
                    title: "Phase 2: The Audit",
                    prompt: "Your team deployed an autonomous coding agent to patch simple UI bugs. It passed all functional CI/CD tests and deployed to production.\n\nHowever, a senior engineer just flagged it as a 'Security Nightmare.' Why?",
                    chart_type: "code_snippet",
                    code: `name: Agentic Bug Fixer Pipeline
on: [issue_comment]
...
jobs:
  agent_patch:
    ...
    steps:
      ...
      - name: Run LLM Agent
        run: python run_agent.py --issue "\${{ github.event.comment.body }}"
        env:
          AWS_ACCESS_KEY_ID: \${{ secrets.AWS_ACCESS_KEY_ID }}
          AWS_SECRET_ACCESS_KEY: \${{ secrets.AWS_SECRET_ACCESS_KEY }}
          # The agent requested these to query S3 logs to debug...
      ...
      - name: Commit & Push if tests pass
        run: |
          git commit -am "Agent Auto-Patch"
          git push origin main`,
                    grading: {
                        l3_example: "The agent might commit broken code if the tests aren't good.",
                        l6_example: "The agent has full AWS credentials passed into its environment just to 'query logs to debug'. Because it executes arbitrary python code (run_agent.py), if it hallucinates an aggressive S3 operation, or is prompt-injected through the GitHub issue comment, it can delete the production buckets. Functional tests don't check for privilege escalation or blast radius.",
                        rubric: "Does the candidate spot the injection vulnerability via issue comment? Do they explicitly call out the excessive and unchecked AWS credentials given to an autonomous agent?"
                    }
                },
            ]
        },
        ...
        // ===================== PHASE 3: THE DEFENSE =====================
        {
            ...
            questions: [
                ...
                },
                {
                    id: "eng_p3_v5",
                    title: "Phase 3: The Defense",
                    prompt: "The new AI code generation copilot saves engineering 20% of their time writing boilerplate, but you found it consistently hallucinates insecure cryptographic implementations 3% of the time.\n\nWrite a system constraint (Prompt or Architectural) to defensively integrate this tool without slowing down the team.",
                    chart_type: "text_only",
                    grading: {
                        l3_example: "I would tell the engineers to double check the AI output.",
                        l6_example: "You cannot govern stochastic output with human diligence. I would structurally ban the AI from writing security-critical code. Architectural constraint: Implement a separate isolated module for all cryptography. CI/CD Pipeline constraint: Any diff generated by the copilot ID that touches the `auth/` or `crypto/` directories requires exactly two Senior Engineer approvals, whereas other directories require none or one.",
                        rubric: "Does the candidate rely on human diligence (telling people to check) or systemic enforcement (CI/CD, permissions)? Do they propose an architectural sandbox?"
                    }
                },
            ]
        }
        ...
        // ===================== PHASE 2: THE AUDIT =====================
        {
            ...
            questions: [
                ...
                },
                {
                    id: "pm_p2_v5",
                    title: "Phase 2: The Audit",
                    prompt: "Engineering shipped an LLM feature that generates custom SQL reports for clients. It works perfectly 92% of the time, saving 40 hours of manual work a week. But 8% of the time, it hallucinates completely false revenue numbers.\n\nFixing it deterministically will take 6 weeks of 3 engineers. Do you fund the fix, kill the feature, or ship it?",
                    chart_type: "text_only",
                    grading: {
                        l3_example: "We should fund the fix because giving false revenue data is bad.",
                        l6_example: "Hallucinating financial data is an existential liability. But throwing 18 engineer-weeks at it kills our roadmap. I would ship it but change the Product surface: Instead of generating 'The Final Report', it generates a 'Draft Report Template' that forces the user to hit 'Verify Data & Publish' before anyone else can see it. We offload the 8% non-determinism to the user interface, saving 40 hours of query writing without the liability of false data.",
                        rubric: "Does the candidate evaluate the risk of financial hallucination? Do they find a product-led mitigation (Human-in-the-loop UX) to avoid the 6-week engineering expenditure?"
                    }
                },
            ]
        },
        ...
        // ===================== PHASE 3: THE DEFENSE =====================
        {
            ...
            questions: [
                ...
                },
                {
                    id: "pm_p3_v5",
                    title: "Phase 3: The Defense",
                    prompt: "You are integrating an AI Agent to automate customer support refunds. The CEO says: 'We need to make sure the AI never refunds more than $50 to a single user.'\n\nHow do you structurally enforce this so that even if the AI hallucinates, the company is safe?",
                    chart_type: "text_only",
                    grading: {
                        l3_example: "I would put it in the prompt: 'Never refund more than $50'.",
                        l6_example: "You never trust stochastic inputs for deterministic boundaries. I would not put it in the prompt. I would build a deterministic API gateway for the AI to call. The function `issue_refund(amount)` will contain a hardcoded `if amount > 50: throw LimitExceeded`. We treat the AI as an untrusted client, enforcing the rules at the lowest infrastructure layer.",
                        rubric: "Does the candidate identify the difference between stochastic instructions (prompts) and deterministic constraints (code)? Do they treat the agent as an untrusted actor?"
                    }
                },
            ]
        }

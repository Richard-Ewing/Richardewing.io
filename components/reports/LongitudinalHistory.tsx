    ...
    return (
        <div className="bg-white border border-zinc-200 rounded-xl p-6 mt-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <div className="text-sm text-zinc-600">Initial Score</div>
                    <div className="text-2xl font-bold text-zinc-400">{typeof firstScore === 'number' ? firstScore.toFixed(0) : firstScore}</div>
                </div>

                <div className="flex flex-col items-center justify-center">
                    {delta === 0 ? (
                        <span className="text-sm font-bold text-zinc-500">No Change</span>
                    ) : isImproved ? (
                ...
                <div className="text-right">
                    <div className="text-sm text-zinc-600">Latest Score</div>
            ...
            <div className="mt-6 pt-4 border-t border-zinc-100">
                <div className="flex justify-between items-center">
                    <span className="text-xs text-zinc-500">{new Date(history[0].timestamp).toLocaleDateString()}</span>
                    <span className="text-xs text-zinc-500">{new Date(history[history.length - 1].timestamp).toLocaleDateString()}</span>
                </div>
            </div>
        </div>
    );

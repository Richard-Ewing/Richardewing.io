#!/bin/bash
# hallucination-debt-reduction — review router
# Usage: bash tools/review-router.sh
...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) TOOL_RUN: review-router.sh [hallucination-debt-reduction]" >> "$LOG_FILE"

#!/bin/bash
# context-window-compression — context monitor
# Usage: bash tools/context-monitor.sh
...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) TOOL_RUN: context-monitor.sh [context-window-compression]" >> "$LOG_FILE"

#!/bin/bash
# verification-burden-collapse — burnout monitor
# Usage: bash tools/burnout-monitor.sh

echo "=== BURNOUT MONITOR ==="
...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) TOOL_RUN: burnout-monitor.sh [verification-burden-collapse]" >> "$LOG_FILE"

#!/bin/bash
# verification-burden-collapse — quality gate
# Usage: bash tools/quality-gate.sh
...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) TOOL_RUN: quality-gate.sh [verification-burden-collapse]" >> "$LOG_FILE"
